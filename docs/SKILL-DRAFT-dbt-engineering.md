...
description: dbt platform engineering methodology — project structure, SQL craft, testing strategy, materializations, governance, CI/CD, performance, and maintenance patterns. Load when building, reviewing, or evolving dbt projects.
user-invocable: false
---

# dbt Engineering — Production Methodology

Opinionated methodology for building and maintaining dbt projects.
Synthesized from dbt Labs guides, Coalesce 2024-2025, and community
production experience. Covers structure through maintenance.
...
unit_tests:
  - name: test_return_rate_zero_returns
    ...
    given:
      - input: ref('dim_users')
        rows:
          - {user_id: 1, total_purchases: 10, total_returns: 0,
             total_spent: 500.00}
    expect:
      rows:
        - {user_id: 1, return_rate_pct: 0.0}
```

**What to unit test:** Business formulas, edge cases (NULL, zero, negative,
division by zero, empty sets), conditional logic (CASE statements).

**Execution order in `dbt build`:** Unit tests -> Models -> Data tests.
Unit test failure blocks model materialization.
...
Requires: `manifest.json` from last successful prod run. Only builds
changed models + downstream dependents. 60%+ faster than full builds.
...
- Pattern appears in 2+ models
- Warehouse-specific SQL varies
- Business logic is independently testable
- Cross-project reuse intended

### Macro Conventions

- Namespace by category: `grant__`, `audit__`, `dq__`, `util__`
- Required args first, optional last
- Always return compiled SQL strings
- Test via `dbt run-operation` or unit test wrapper models
...
- Don't Jinja-ify trivial SQL. Jinja adds complexity — justify it.
- `{% set %}` for local model state. `{{ var() }}` for CLI/project injection.
- `{{ env_var() }}` for secrets — never hardcode credentials.
- Whitespace control (`{%- -%}`) only if causing SQL syntax errors. Readability wins.
- Debug: `dbt compile --select my_model` then inspect `target/compiled/`.

### Snapshots

- **Timestamp strategy** (preferred): requires reliable `updated_at` column
- **Check strategy**: when timestamps unreliable. Specify columns — never `check_cols: all`
- YAML-based definition (dbt 1.9+), not SQL blocks
- Test: `dbt_scd_id` not_null, `dbt_valid_from` not_null
...
5. **Partial parsing**: Automatic in dbt 1.4+. Keep `target/` between runs.
6. **Slim CI**: `state:modified+` — never full-build in CI

### Finding Slow Models

```bash
# After dbt build, analyze run_results.json
...
1. Add `alias` to old model pointing to new table name
2. Create new model file with new name
...
1. Create new model with incremental config alongside old view
2. Backfill: `dbt run --full-refresh --select new_model`
3. Validate with `dbt-audit-helper` (compare row counts, column values)
4. Swap downstream refs
5. Remove old model
...
**Version pinning**: Exact version for production (`version: 1.2.0`).
Range for development (`version: [">=1.2.0", "<1.3.0"]`).
Always commit `package-lock.yml`.
